...
void enter(enum idform f, enum datatype tk, int lev, int *tx, int *off)
{
    ...
    if (tk == inttype)
        ...
        (*off) = (*off) + 1;
    // else if(tk==funkind)
    //  table[(*tx)].address =-1;      //仅仅为了避免打印出来的函数地址错觉，以-1显示
}
...
int stmts(int lev, int *tx, int *off)
{
    ...
    while (token.kind == id || token.kind == iftoken || token.kind == lbrace || token.kind == readtoken || token.kind == writetoken)
    {
...
int stmt(int lev, int *tx, int *off)
{
    ...
    else if (token.kind == readtoken) // 准备按照return语句处理
    {
        READstmt(lev, tx, off);
    }
...
struct addrRecord *factor(int *tx, int lev)
{
    ...
    else if (token.kind == num) //因子为整数常量 */
    {
        ...
        // printf("-----%d",temp->constant.value);=NextIR;    //记录不完整的指令
        gettoken();
        // printf("--%d\n",sym);
    }
